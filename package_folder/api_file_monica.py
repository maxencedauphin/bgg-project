...
xgboost_model_path = os.path.join(models_path, "all_models_20250406-182538.pkl")

# Load the model
with open(xgboost_model_path, "rb") as file:
    xgboost_optimised = pickle.load(file)
...
# To make a prediction, the model must have as input the same number of features/columns filled as those used to train it
    ...
    required_columns = xgboost_optimised.feature_names_in_  # Storing all the names of the model features/columns (used to train the model) in the required_columns variable
    ...
    invalid_mechanics = [mech for mech in mechanics if mech not in required_columns]
    if invalid_mechanics:
        return {"ERROR": f"Invalid mechanics: {invalid_mechanics}"}

    # Mark mechenics present
    for mech in mechanics:
        model_input_data[mech] = 1

    # Every column necessary is present
    missing_cols = [col for col in required_columns if col not in model_input_data.columns]
    if missing_cols:
    ...
    # Make the prediction
    pred = xgboost_optimised.predict(model_input_data)[0]
